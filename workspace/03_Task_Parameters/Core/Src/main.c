...
const uint16_t *pGreenLed = (uint16_t *)GREEN;
const uint16_t *pOrangeLed = (uint16_t *)ORANGE;
const uint16_t *pRedLed = (uint16_t *)RED;
const uint16_t *pBlueLed = (uint16_t *)BLUE;
...
/**
 * @brief Task handler demonstrating parameter passing to FreeRTOS tasks.
 * @retval None
 */
void vLedControllerTask(void *pvParameters)
{
	...
	while (1)
	{
		...
		/* Delay to allow observation through the SFR window. */
		for (i = 0; i < 60000; i++)
		{
			/* Do nothing */
		}
